...
public class DiverSim extends Applet implements Runnable
{
   ...
   *****************************************************************/
   ...
   private int ticks_per_fish1;
   private int ticks_per_fish0;
   private int ticks_per_shark;
   private int shark_frames;
   ...
   public void init()
   {
      ...
      // This parameter specifies the size of the array which will store
      // fish positions
      max_no_fish = 40;
      ...
      // Images to use for a shark
      shark_frames = 2;
      shark = new Image[shark_frames];
      for(x=0; x<shark_frames; x++)
      {
         shark[x]=getImage(getCodeBase(),"images/shark"+x+".gif");
      }

      // Images to use for two fish
      fish0 = getImage(getCodeBase(),"images/fish0.gif");
      fish1 = getImage(getCodeBase(),"images/fish1.gif");

      // Number of ticks before drawing two types of fish or a shark
      ticks_per_fish1 = 73;
      ticks_per_fish0 = 82;
      ticks_per_shark = 100;

      // Array of fish classes to use for drawing them
      fish = new Fish[max_no_fish];
   ...
   public void draw_sim(int tick_no)
   {
      ...
      for(x=0; x<max_no_fish; x++)
      {
         if (fish[x] != null)
         {
            if (fish[x].type.equals("Fish0"))
            {
               buffer_screen.drawImage(fish0
                                      ,fish[x].x_coord
                                      ,fish[x].y_coord
                                      ,this);
            }
            else if (fish[x].type.equals("Fish1"))
            {
               buffer_screen.drawImage(fish1
                                      ,fish[x].x_coord
                                      ,fish[x].y_coord
                                      ,this);
            }
            else if (fish[x].type.equals("Shark"))
            {
               buffer_screen.drawImage(shark[fish[x].frame]
                                      ,fish[x].x_coord
                                      ,fish[x].y_coord
                                      ,this);
            }
   ...
   private void process_objects(int tick_no, int diver_x, int diver_y)
   {
      ...
      // if this tick is an appropriate number, then create a fish1
      if (tick_no%ticks_per_fish1 == 0)
      {
         for(x=0; x<max_no_fish; x++)
         {
            // try to find an empty slot in the fish array
            if (fish[x] == null)
            {
               fish[x] = new Fish(Xdiver_size
                                 ,(int) (Math.random()*Ydiver_size+bordersize)
                                 ,"Fish1"
                                 ,1);
               break;
            }
      ...
      // if this tick is an appropriate number, then create a fish0
      if (tick_no%ticks_per_fish0 == 0)
      {
         for(x=0; x<max_no_fish; x++)
         {
            // try to find an empty slot in the fish array
      ...
      // if this tick is an appropriate number, then create a shark
      if (tick_no%ticks_per_shark == 0)
      {
         for(x=0; x<max_no_fish; x++)
         {
            // try to find an empty slot in the fish array
      ...
      // Now we have created all the fish where appropriate, so call the
      // tick methods in all pre-existing objects to increment their position
      // on the screen
      for(x=0; x<max_no_fish; x++)
      {
         if (fish[x] != null)
         {
            ...
            // if this fish has reached the left hand side of the screen remove
            // it
            if(fish[x].x_coord <= bordersize)
            {
               fish[x] = null;
            }
         }
